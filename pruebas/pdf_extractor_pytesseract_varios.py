...
def extract_text_from_pdf(input_path, output_path, processed_folder):
    ...
    try:
        ...
        # Inicializar PyTesseract con el idioma deseado (opcional)
        # pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'  # Ruta al ejecutable de Tesseract

        # Iterar sobre cada imagen y extraer el texto
        text = ""
        for idx, img in enumerate(images):
            # Guardar la imagen temporalmente (si es necesario)
            # img.save(f"page_{idx}.jpg", 'JPEG')

            # Aplicar OCR a la imagen
            text = text + pytesseract.image_to_string(Image.open(f"page_{idx}.jpg"), lang='spa')  # Español

            # Eliminar la imagen temporal
            # os.remove(f"page_{idx}.jpg")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Uso: python pdf_extractor_pytesseract_varios.py <carpeta_entrada> <carpeta_salida>")
        sys.exit(1)
